...
export function displayMessage(ctx, message, color = "red") {
  ctx.font = "30px Arial";
  ctx.fillStyle = color;
...
export function isHeroDead() {
  return new Hero().life <= 0;
}

export function isEnemiesDead() {
  const enemies = GameObjects.getInstance.list.filter((go) => go.type === "Enemy" && !go.dead);
  return enemies.length === 0;
}

export function endGame(win) {
  console.log('end game...')
  cancelAnimationFrame(GameManager.requestAnimationFrameId)
  GameManager.requestAnimationFrameId = -1;
 ...
 export function resetGame() {
  const eventEmitter = new EventEmitter();
  if (GameManager.requestAnimationFrameId) {
    cancelAnimationFrame(GameManager.requestAnimationFrameId);
    eventEmitter.clear();
    initGame();
    GameObjects.getInstance.list = [];
  }
 }